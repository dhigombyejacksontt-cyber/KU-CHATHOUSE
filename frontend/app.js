...
function initAuthenticatedPage(page) {
    fetch('/api/auth/me')
        ...
        .then(data => {
            ...
            highlightActiveSidebarLink(page);
...
function highlightActiveSidebarLink(page) {
...
function populateProfileUI(user) {
    ...
    if (navRole) {
        navRole.innerText = user.category;
    ...
    setAvatarElement(sidebarAvatar, user.username, user.profile_photo);
    if (sidebarUsername) sidebarUsername.innerText = user.username;
    if (sidebarRole) sidebarRole.innerText = user.category;
    if (sidebarId) sidebarId.innerText = user.university_id;
    if (sidebarFaculty) sidebarFaculty.innerText = user.faculty;
    if (sidebarBio) {
        if (user.bio) {
            sidebarBio.textContent = user.bio;
            sidebarBio.classList.remove("bio-empty");
        } else {
            sidebarBio.textContent = "No bio yet -- tell people a bit about yourself.";
            sidebarBio.classList.add("bio-empty");
        }
    }
    ...
    if (user.category === 'Administrator' && !document.getElementById('staff-directory-btn')) {
        const sidebar = document.querySelector('.sidebar-menu');
        if (sidebar) { const btn = document.createElement('button'); btn.id='staff-directory-btn'; btn.className='sidebar-menu-item staff-directory-btn'; btn.type='button'; btn.innerHTML='<i class="fa-solid fa-id-badge"></i> Staff Directory'; btn.onclick=openStaffDirectory; sidebar.appendChild(btn); }
    }
}
...
function populateOwnProfileForm(user) {
    ...
    if (idDisplay) idDisplay.value = user.university_id;
    if (bioInput) {
...
function handleCategoryChange() {
    ...
    if (idInput) idInput.placeholder = category === 'Student' ? 'e.g. KU-2026-0012' : category === 'Lecturer' ? 'e.g. KU-0047 (approval required)' : category === 'Administrator' ? 'e.g. KU-0012 (approval required)' : 'Select your role first';
...
function handleRegister(e) {
    ...
    // Any category (Student, Lecturer, Administrator) just needs a valid KU-issued ID -- no more
    // category-specific STD/LEC/ADM prefix requirement.
    if (!university_id.toUpperCase().startsWith('KU')) { showToast('University ID must start with KU', 'error'); return; }

    // Password must start with KU too (replaces the old STD/LEC-prefixed password scheme --
    // it's now one consistent rule regardless of category).
    if (!password.toUpperCase().startsWith('KU')) { showToast('Password must start with KU (e.g. KU2026Secure)', 'error'); return; }
