...
// ==========================================================================
// PROFILE MODAL (change username / bio / upload profile photo)
...
function openProfileModal() {
    ...
    if (idDisplay) idDisplay.value = currentUser.university_id;
    if (bioInput) {
...
function handleSaveProfile(e) {
    ...
    avatarUpload
        ...
        .then(data => {
            ...
            closeProfileModal();
            // Refresh whichever list is on the current page
            loadPosts();
...
function handleCategoryChange() {
    ...
    if (idInput) idInput.placeholder = category === 'Student' ? 'e.g. KU-STD-2026-0012' : category === 'Lecturer' ? 'e.g. KU-LEC-0047 (approval required)' : category === 'Administrator' ? 'e.g. KU-ADM-0012 (approval required)' : 'Select your role first';
...
function handleRegister(e) {
    ...
    const requiredPrefix = category === 'Student' ? 'KU-STD-' : category === 'Lecturer' ? 'KU-LEC-' : 'KU-ADM-';
    if (!university_id.toUpperCase().startsWith(requiredPrefix)) { showToast(`${category} ID must start with ${requiredPrefix}`, 'error'); return; }
...
function renderSearchDropdown(members, posts, query) {
    ...
    if (topMembers.length) {
        ...
        html += topMembers.map(m => `
            <div class="search-result-row" onclick="window.location.href='/members?search=${encodeURIComponent(m.username)}';">
                <div class="mini-avatar${m.profile_photo ? ' has-photo' : ''}">${avatarInnerMarkup(m.username, m.profile_photo)}</div>
                <div class="member-text">
                    <span class="member-username">${escapeHTML(m.username)}</span>
