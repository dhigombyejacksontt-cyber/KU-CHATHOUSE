...
def register():
    ...
    id_prefix = {'Student': 'KU-STD-', 'Lecturer': 'KU-LEC-', 'Administrator': 'KU-ADM-'}[category]
    if not university_id.upper().startswith(id_prefix):
        return jsonify({"error": f"{category} ID must start with {id_prefix}"}), 400
...
def manage_staff():
    ...
    expected = 'KU-LEC-' if role == 'Lecturer' else 'KU-ADM-' if role == 'Administrator' else None
    if not expected or not staff_id.startswith(expected): return jsonify({"error": "Invalid staff ID format for the selected role."}), 400
